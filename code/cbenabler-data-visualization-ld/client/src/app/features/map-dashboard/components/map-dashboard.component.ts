...
export class MapDashboardComponent extends BaseComponent implements AfterViewInit, OnDestroy {
    ...
    private colors: any = {low:"#28a746",medium:"#ffc107",high:"#dc3546"};
    private intervalRefreshMilliseconds: number = 60000;
    private entityAttr: string = 'data';
    private map: L.Map;
    private latCenter: number = 37.3984397;
    private longCenter: number = -5.9881996;
    ...
    private defaultZoom: number = 15.5;
    ...
    private processModels(models: ModelDto[]): void {
        models.forEach((model, i) => {
            ...
            model.data.forEach(entity =>{
                this.addEntity(model, entity, categoryKey, i)
            });
    ...
    *****************************************************************************/
     ...
     private getColor(index: number): string{
        ...
        }else if(index >= 170 && index < 334){
            color = this.colors.medium;
        }else if(index >= 334 && index < 500){
    ...
    private setGeoJSONattributes(model: ModelDto, entity: Entity): string{
        var color: string;
        color = this.getColor(entity.peopleOccupancy);
